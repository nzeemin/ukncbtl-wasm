...
const uint32_t Emulator_CanvasGrayColors[16 * 8] =
{
...
extern "C" {
    ...
    EMSCRIPTEN_KEEPALIVE void* Emulator_PrepareScreen(int screenViewMode)
    {
        //printf("Emulator_PrepareScreen(%d)\n", screenViewMode);
        if (g_pFrameBuffer == 0)
            printf("Emulator_PrepareScreen() null framebuffer\n");

        const uint32_t* palette = Emulator_CanvasGRBAColors;
        switch ((ScreenViewMode)screenViewMode)
        {
            case RGBScreen:
                palette = Emulator_CanvasRGBAColors;  break;
            case GrayScreen:
                palette = Emulator_CanvasGrayColors;  break;
            case GRBScreen:
                palette = Emulator_CanvasGRBAColors;  break;
        }

        Emulator_PrepareScreenRGB32(g_pFrameBuffer, palette);
